#!/usr/bin/env python3
"""
Test script with Corrected MockAPI.io Integration
"""
...
def test_complete_flow_with_corrected_api():
    """Test the complete flow with corrected MockAPI.io endpoint"""
    
    print("🚀 Testing Complete Flow with Corrected API...")
    print("=" * 50)
    ...
    # Use the correct MockAPI.io endpoint (without /api/v1)
    api_url = "https://68d39755214be68f8c6666a0.mockapi.io"
    ...
    try:
        ...
        print("✅ Complete Flow Results with Corrected API:")
        ...
        if result['success']:
            ...
            if api_stage['successful_creations']:
                ...
                for i, creation in enumerate(api_stage['successful_creations'][:5]):  # Show first 5
            ...
            if api_stage['failed_creations']:
                ...
                for i, failed in enumerate(api_stage['failed_creations'][:3]):  # Show first 3
                    ...
                    print(f"      Error Type: {failed.get('error_type', 'N/A')}")
                    print(f"      Status Code: {failed.get('status_code', 'N/A')}")
                print()
...
def test_single_customer_creation():
    """Test creating a single customer to debug the API format"""
    ...
    # Sample customer from our transformation
    sample_customer = {
        "name": "Acme Corp",
    ...
    try:
        print(f"📤 Creating customer: {sample_customer['name']}")
        ...
        if response.status_code in [200, 201]:
            print("🎉 Single customer creation successful!")
            return True
        else:
            print(f"❌ Single customer creation failed: {response.status_code}")
            return False
            
    except Exception as e:
        print(f"❌ Single customer creation failed: {str(e)}")
        return False

if __name__ == "__main__":
    # Test single customer creation first
    single_success = test_single_customer_creation()
    
    if single_success:
        # Test complete flow with corrected API
        result = test_complete_flow_with_corrected_api()
        ...
        print("\n💥 Single Customer Creation Test Failed")
